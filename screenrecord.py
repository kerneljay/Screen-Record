...
stop_flag = False
replace_mode = False
record_region = None  # NEW: (x, y, width, height) or None for full screen
...
def save_config(path, replace_mode=False, record_region=None):
    config = {
        "save_path": str(path), 
    ...
    if CONFIG_FILE.exists():
        try:
            with open(CONFIG_FILE, "r") as f:
                ...
                return path, replace_mode, record_region
        except Exception:
            return None, False, None
    return None, False, None

def delete_old_recordings():
    """Delete all existing recordings in the save directory"""
...
def select_region():
    ...
    def end_select(event):
        ...
        if is_selecting:
            ...
            if width > 10 and height > 10:  # Minimum size check
                ...
                overlay.destroy()
                root.deiconify()
                region_label.config(text=f"Region: {width}x{height} at ({x1},{y1})")
                save_config(save_path, replace_mode, record_region)
    ...
    # Instructions
    instruction_label = tk.Label(overlay, text="Drag to select recording region • ESC to cancel • Enter for full screen", 
    ...
    overlay.bind("<Return>", lambda e: set_fullscreen_region())
    
    def set_fullscreen_region():
        screen_size = pyautogui.size()
        record_region = (0, 0, screen_size[0], screen_size[1])
        overlay.destroy()
        root.deiconify()
        region_label.config(text=f"Region: Full Screen ({screen_size[0]}x{screen_size[1]})")
        save_config(save_path, replace_mode, record_region)
        messagebox.showinfo("Region Selected", "Recording region set to full screen")
    
    overlay.focus_set()

def clear_region():
    """Clear the selected region and return to full screen recording"""
    global record_region
    record_region = None
    region_label.config(text="Region: Full Screen (auto)")
    save_config(save_path, replace_mode, record_region)
    messagebox.showinfo("Region Cleared", "Recording region cleared. Will record full screen.")


saved_path, saved_replace_mode, saved_record_region = load_config()
...
def record_screen(duration, fps=30):
    ...
    # NEW: Delete old recordings if replace mode is enabled
    if replace_mode:
    ...
    # NEW: Hide window during recording
    ...
    if record_region:
        ...
        print(f"[+] Recording full screen: {width}x{height}")
    ...
    # Update status (even though window is hidden, tray tooltip can show this)
    status_text = f"Recording {'region' if record_region else 'full screen'}..."
    ...
    try:
        ...
        # Restore window if it was visible before recording
        if was_visible:
            root.deiconify()
            root.state('normal')
            root.lift()
    ...
    region_text = " (Region)" if record_region else " (Full Screen)"
...
def toggle_replace_mode():
    """Toggle between replace mode and accumulate mode"""
    global replace_mode
    replace_mode = not replace_mode
    
    # Update button text and color
    if replace_mode:
        replace_toggle_btn.config(text="📁 Replace Mode: ON", bg="orange", fg="white")
        mode_info = "Replace Mode: New recordings will delete old ones"
    else:
        replace_toggle_btn.config(text="📁 Replace Mode: OFF", bg="lightblue", fg="black")
        mode_info = "Accumulate Mode: Keep all recordings"
    
    # Save the setting
    save_config(save_path, replace_mode, record_region)
    
    # Show info message
    messagebox.showinfo("Mode Changed", mode_info)
...
def toggle_window_visibility():
    """Toggle window between hidden and visible states"""
    try:
...
def delete_all_recordings():
    """Delete all recordings in the save directory"""
...
root.geometry("380x600")  # Made taller for region controls
root.resizable(False, False)

# Add hotkey info at the top
...
region_text = "Region: Full Screen (auto)"
if record_region:
    x, y, w, h = record_region
    region_text = f"Region: {w}x{h} at ({x},{y})"

region_label = tk.Label(region_frame, text=region_text, bg='lightyellow', 
...
tk.Button(region_btn_frame, text="🖥️ Clear Region", command=clear_region, 
          bg="lightcoral", font=('Arial', 8)).pack(side='left', padx=2)

# Replace mode toggle button
replace_toggle_btn = tk.Button(root, text="📁 Replace Mode: OFF", command=toggle_replace_mode, 
